...
function App() {
  ...
  const [score, setScore] = useState(() => {
    const storedScore = localStorage.getItem('rps_score_original')
  ...
  useEffect(() => {
    localStorage.setItem('rps_score_original', score)
  }, [score])
  ...
  return (
    <>
      {/* Rules Modal */}
      <div
  className={`transition-all md:duration-300 duration-600 ease-initial 
  ${showRules ? "md:w-[28dvw] w-[100dvw]  md:h-[28dvw] h-[100dvh] z-50" : "opacity-0 pointer-events-none absolute z-0"} bg-white md:w-[28dvw] w-[100dvw]  md:h-[28dvw] h-[100dvh]  md:rounded-[1dvw] rounded-none md:py-0 py-[8dvw] md:fixed md:transform md:-translate-x-1/2 md:-translate-y-1/2 md:left-1/2 md:top-1/2 flex flex-col md:justify-start justify-between items-center`}>
          ...
          <div className='flex items-center justify-center md:h-[65%] h-[80%]'>
            <img src="/images/rules.svg" alt="rules" className='md:w-[18dvw] xm:w-[84dvw] w-[74dvw] h-fit mx-auto' />
          </div>
...
<header className='md:w-[52vw] w-[75vw] md:mt-[2dvw] mt-[5dvw] rounded-[1.4dvw] p-[2dvw] border border-header-outline flex justify-between items-center'>
  <img
    src="/images/logo.svg"
    alt="logo"
    className='w-[17vw] md:w-[11dvw] h-auto'
  />
...
<section className='w-full md:h-[35dvw] h-[121dvw] flex flex-col items-center justify-start'>
  ...
  <button className={`cursor-pointer barlow-600 md:tracking-widest tracking-[0.4dvw] ${location.pathname==="/"? "bg-red-700" : ""} border border-[#a8a8a8] md:rounded-[0.4dvw] rounded-[1dvw] text-white cursor-pointer hover:brightness-110 md:px-[0.5dvw] md:py-[0.1dvw] px-[1dvw] py-[0.2dvw]`} onClick={navigateToOriginal}>ORIGINAL</button>

  <button className={`${location.pathname==="/bonus"? "bg-red-700" : ""} border border-[#a8a8a8] md:rounded-[0.4dvw] rounded-[1dvw] text-white hover:brightness-110 cursor-pointer m-auto md:px-[0.5dvw] md:py-[0.1dvw] px-[1dvw] py-[0.2dvw] barlow-600 md:tracking-widest tracking-[0.4dvw]`} onClick={navigateToBonus}>BONUS</button>
  </span>

{/* Triangle Buttons Layout */}
<div className={`${playerPick ? "hidden" : "flex"} relative md:w-[30dvw] w-[62vw]  aspect-[13/12] mx-auto  md:mt-[2dvw] mt-[0dvw]`}>
  {/* Paper - Top Left */}
  <span
    onClick={() => handlePick("paper")}
    className="rounded-full absolute top-0 left-0 md:translate-x-[-27%] md:translate-y-[-27%] hover:brightness-103 scale-100 md:scale-46 transition-transform duration-200"
  >
    <Paper />
  </span>
  ...
  <span
    ...
    className="rounded-full absolute top-0 right-0 md:translate-x-[27%] md:translate-y-[-27%] scale-100 md:scale-46 transition-transform duration-200 hover:brightness-103"
  >
    <Scissors />
  </span>
  ...
  <span
    ...
    className="rounded-full absolute bottom-0 left-1/2 -translate-x-1/2 md:translate-y-[25%] scale-100 md:scale-46 transition-transform duration-200 hover:brightness-103"
  >
    <Rock />
  </span>
</div>
          ...
          <div className={`flex md:gap-0 gap-[8dvw] w-full justify-center ${playerPick ? "visible" : "hidden"} md:mt-[2dvw] mt-[0dvw]`}>
            ...
            {/* Result */}
            <div
            className={`text-white  mt-[11dvw] h-[15dvw] md:flex flex-col justify-center gap-[0.4dvw] barlow-700 tracking-widest whitespace-nowrap px-[4dvw] md:visible hidden ${result ? "w-[26dvw] opacity-100 pointer-events-auto animate-pop" : "w-0 opacity-0 pointer-events-none"}`}>
    ...
    <div className={`z-[9999] bg-radial border-[0.22dvw] border-gray-200 text-white p-[2dvw] rounded-[0.6dvw] max-w-[30dvw] text-center barlow-600 tracking-wider ${showModal? "animate-pop" : ""}`}>
      ...
      <div className="flex justify-center gap-[2dvw]">
        ...
        >
          ...
          {/* Result */}
          <div className={`text-white md:mt-[15dvw] mt-[14dvw] md:hidden flex flex-col justify-center barlow-700 tracking-widest whitespace-nowrap  ${result ? "animate-pop opacity-100  w-full h-[20dvw]" : " w-full h-[20dvw] opacity-0"}`}>
      ...
      <div className="flex justify-center gap-[2dvw]">
        ...
        <footer className='w-screen flex md:justify-end justify-center md:mr-[5dvw] pb-[5dvw]'>
          <button onClick={() => (setShowRules(true),
            clickSound())} className='cursor-pointer w-fit barlow-600 md:rounded-[0.4dvw] rounded-[2dvw] text-[#e6e6e6] md:tracking-[0.2dvw] tracking-[0.8dvw] md:py-[0.5dvw] xm:py-[2dvw] py-[1dvw] md:px-[2dvw] xm:px-[6dvw] px-[4dvw] md:border-[0.1dvw] border-[0.1px] my-auto border-[#a8a8a8] hover:brightness-120'>RULES</button>
        </footer>
